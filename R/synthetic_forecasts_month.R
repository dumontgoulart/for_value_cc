#Multiple season: one per month and considering the observed data wrt to the previous month of forecasted 
#################################################################
KNN3.ma <- function(cur.obs,all.obs,all.residuals,yest.residual,two.yest.residual,three.yest.residual, yest.obs, two.yest.obs, three.yest.obs, kk) {
  ...
  dist5 <-(0*((all.obs-yest.obs)/sd(all.obs))) ^2
  dist6 <-(0*((all.obs-two.yest.obs)/sd(all.obs))) ^2
  dist7 <-(0*((all.obs-three.yest.obs)/sd(all.obs))) ^2
  
  distance <- sqrt( dist1 + dist2 + dist3 + dist4 + dist5 +dist6 +dist7) 
...
#################################################################
prcp_synthetic_forecast_month <- function(num.sim,obs.period.data,fore.period.data) {
  ...
  kk <- 7 #round(sqrt(n.reforecast/12))     #K in KNN algorithm. we divide by 2 since we resample separately by season
  ...
  final.synthetic.forecasts <- data.frame('date'=obs.period.data$date, 'season'=obs.period.data$season, 'obs'=obs.period.data$obs,simulated.forecasts, 'res'=log(obs.period.data$obs) - log(simulated.forecasts))

...
prcp_synthetic_forecast <- function(num.sim,obs.period.data,fore.period.data) {
  ...
  sim.fore <- array(NA,c(n.obs,num.sim))  
  sim.resid <- array(NA,c(n.obs,num.sim))  
  for (i in 1:num.sim) {    #loop through different synthhetic traces
    for (j in 1:n.obs) {        #loop through each observation for which we will develop a synthetic forecast
      ...
      sim.resid[j,i] <- KNN(cur.obs,fore.period.data.cur$obs,fore.period.data.cur$residual,kk)   #resample a residual for the current observation
      sim.fore[j,i] <- cur.obs*exp(sim.resid[j,i])   #add the residual to the current observation to develop a synthetic forecast
    }
  }
  
  final.synthetic.forecasts <- data.frame('date'=obs.period.data$date,'season'=obs.period.data$season,'obs'=obs.period.data$obs,'fore'=sim.fore, 'resid'=sim.resid)

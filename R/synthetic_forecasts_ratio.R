...
#################################################################
prcp_synthetic_forecast_rat <- function(num.sim,obs.period.data,fore.period.data) {
  ...
  for (i in 1:num.sim) {    #loop through different synthhetic traces
    for (j in 1:n.obs) {        #loop through each observation for which we will develop a synthetic forecast
      ...
      simulated.forecasts[j,i] <- exp( log(cur.obs) / (cur.resid)  )  #add the residual to the current observation to develop a synthetic forecast
    }
  }
  
  final.synthetic.forecasts <- data.frame('date'=obs.period.data$date,'obs'=obs.period.data$obs,simulated.forecasts)

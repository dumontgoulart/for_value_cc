...
#################################################################
KNN2 <- function(cur.obs,all.obs,all.residuals,yest.residual,two.yest.residual,kk) {
  ...
  distance <- sqrt(((all.obs-cur.obs)/sd(all.obs)) ^2 + (1*((all.residuals - yest.residual)/sd(all.residuals))) ^2 + (1*((all.residuals - two.yest.residual)/sd(all.residuals))) ^2) 
...
#################################################################
prcp_synthetic_forecast_dif_lag <- function(num.sim,obs.period.data,fore.period.data) {
  ...
  for (i in 1:num.sim) {    #loop through different synthhetic traces
    for (j in 1:n.obs) {        #loop through each observation for which we will develop a synthetic forecast
      ...
      cur.resid <- KNN2(cur.obs,fore.period.data.cur$obs,fore.period.data.cur$residual,yest.residual,two.yest.residual,kk)   #resample a residual for the current observation

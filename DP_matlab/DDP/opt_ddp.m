...
for t = N: -1: 1
  ...
  for i = 1: n_s; % for each discretization level of state 
    ...
    sys_param.simulation.vv = interp_lin_scalar( discr_q , min_rel( i, :, doy ) , disturbance(t) );
    sys_param.simulation.VV = interp_lin_scalar( discr_q , max_rel( i, : ) , disturbance(t) );
